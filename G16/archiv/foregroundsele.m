...
%% main function
function [out,rectout,mask] = foregroundsele(img)
% Preparation
[M,N,D] = size(img);
%figure, imshow(img)
k = 0;
...
hold on
while 1
    [x,y,flag] = ginput(1); % returns the mouse button used to select each point
    if flag == 1 % confirm whether it is the left mouse button input
        k = k + 1;
        plot(x,y,'b.','MarkerSize',20)
        p(k,1:2) = round([y,x]);
        if k>1
           line([p(k-1,2),p(k,2)],[p(k-1,1),p(k,1)],'LineWidth',2) % connect two adjacent points
        end
    elseif flag == 32 % if it is the space button input, cancel this selection
        k = 0;
        p = [];
        out = 0;
        rectout = 0;
        ...
        line([p(1,2),p(k,2)],[p(1,1),p(k,1)],'LineWidth',2) % Connect the first point and the last point
        break
    end
end
hold off
